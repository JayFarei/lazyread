...
async function renderReader(id: string): Promise<void> {
  ...
  <div class="reader-dock" id="reader-dock"><aside class="shortcut-hud" aria-label="Keyboard shortcuts"><span>Shortcuts</span><dl><div><dt>H</dt><dd>mark</dd></div><div><dt>⇧ H</dt><dd>prior</dd></div><div><dt>U</dt><dd>undo</dd></div><div><dt>↵</dt><dd>play / pause</dd></div><div><dt>P</dt><dd>next ¶</dd></div><div><dt>⇧ P</dt><dd>prior ¶</dd></div><div><dt>W</dt><dd>next word</dd></div><div><dt>B</dt><dd>prior word</dd></div></dl></aside>
    ...
    <div class="speed-menu liquid" id="speed-menu" hidden>${[0.8,1,1.15,1.3,1.5,1.75,2].map((rate) => `<button data-rate="${rate}" aria-pressed="${rate === 1}">${rate}×</button>`).join("")}</div>
...
async function setupPlayer(article: Article, articleBody: HTMLElement): Promise<void> {
  ...
  const update = (): void => { const position = findWordAtTime(words, audio.currentTime); if (position >= 0) { const timing = words[position]!; activeIndex = timing.index; const next = wordElements.get(activeIndex); if (next && next !== activeElement) { activeElement?.classList.remove("is-current"); activeElement = next; next.classList.add("is-current"); const heading = [...articleBody.querySelectorAll("h2, h3")].filter((item) => item.compareDocumentPosition(next) & Node.DOCUMENT_POSITION_FOLLOWING).at(-1); required("#current-section").textContent = heading?.textContent ?? article.title; if (!audio.paused) reveal(next); } } currentTime.textContent = formatTime(audio.currentTime); timeline.value = String(audio.duration ? Math.round(audio.currentTime / audio.duration * 1000) : 0); timeline.style.setProperty("--progress", `${Number(timeline.value) / 10}%`); if (!audio.paused) frame = requestAnimationFrame(update); };
  const seek = (index: number): void => { const timing = words.find((word) => word.index === index); const element = wordElements.get(index); if (!timing || !element) return; const previous = audio.currentTime; undo.push(() => { audio.currentTime = previous; update(); }); audio.currentTime = timing.start; activeIndex = index; update(); reveal(element); };
  const toggle = async (): Promise<void> => { if (play.disabled) return; if (audio.paused) await audio.play(); else audio.pause(); };
  play.addEventListener("click", () => void toggle()); audio.addEventListener("play", () => { play.innerHTML = icon("pause"); play.ariaLabel = "Pause article"; cancelAnimationFrame(frame); frame = requestAnimationFrame(update); }); audio.addEventListener("pause", () => { play.innerHTML = icon("play"); play.ariaLabel = "Play article"; cancelAnimationFrame(frame); update(); }); audio.addEventListener("ended", () => { play.innerHTML = icon("retry"); play.ariaLabel = "Replay article"; });
